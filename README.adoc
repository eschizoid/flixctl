...
Inspired by `kubectl`, `flixctl` aims to provide a simple API for provision the required infrastructure for a true MaaS
(Movies as a Service) architecture. All flixctl commands were abstracted using Slack Slash API and backing them up by AWS
Lambda. The flexibility of this architecture allows an easy installation of the full API into any Slack workspace.
