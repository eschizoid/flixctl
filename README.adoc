...
Inspired by `kubectl`, `flixctl` aims to provide a CLI for controlling the infrastructure necessary for a true MaaS (Movies
as a Service) architecture. All flixctl commands were conceived with the idea to be later abstracted using Slack Slash
API, making the full CLI available from any Slack workspace wishing to install it.
